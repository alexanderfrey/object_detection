...
def translate_coordinates(porg):
    ...
    plat = porg['olat'] + yy / METERS_DEGLAT(porg['olon'])

    return np.array([plat, plon])


def transmit_box(boxes):
    adjusted_coordinates = np.empty((0,2))
    for box in boxes:
        ymin, xmin, ymax, xmax = box
        porg = {}
        porg['x'] = ((xmax - xmin) / 2) * 50
        porg['y'] = ((ymax - ymin) / 2) * 50
        porg['rotation_angle_degs'] = 0
        porg['xoffset_mtrs'] = 0
        porg['yoffset_mtrs'] = 0
        porg['olon'] = 51.505
        porg['olat'] = -0.09
        sll = translate_coordinates(porg)
        adjusted_coordinates = np.append(adjusted_coordinates, [sll], axis=0)
        logging.info('sll %s', sll)

    logging.info('adjusted_coordinates %s', adjusted_coordinates.tolist())
    #data = {'xmin': xmin, 'ymin': ymin, 'xmax': xmax, 'ymax': ymax}

    socketIO.emit('send:boxes',{"id": 1, 'boxes': adjusted_coordinates.tolist()})
...
def visualize_boxes_and_labels_on_image_array(image,
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    if scores is None or scores[i] > min_score_thresh:
      ...
      new_boxes = np.append(new_boxes, [box], axis=0)
